...
import { listSources, resolveSource, summarySources, type ResolvedSource } from "@/lib/general-report/sources";
...
export async function GET(req: Request) {
  try {
    ...
    const sources = await listSources(me);
    if (sources.length === 0) {
    ...
    if (source === "summary") {
      const parts: ResolvedSource[] = await summarySources(me);
    ...
    const resolved = await resolveSource(me, source);
    if (!resolved) {
